...
#
    ...
    def loadaneos(self, aneosinfname=None, aneosoutfname=None, silent=False, includetension=False, setrefs=False):
        """Function for reading in ANEOS INPUT and OUTPUT FILE DATA into EOS structure."""
        """RIC added ability to include tension region of melt/vapor curve"""
        """RIC added option to set reference values from ANEOS file"""
        ...
        for i in np.arange(len(testin)):
            if testin[i].find('ANEOS') == 0:
                ...
                if testin[i].find('ANEOS2') == 0:
                    ...
                    #print('rho0=',rho0)
                    #RIC if setrefs then read in P0, K0, T0
                    if setrefs == True:
                        T0=float(tmp[40:50])
                        P0=float(tmp[50:60])
                        K0=float(tmp[60:70])
        ...
        #RIC if setrefs == True then initialize R0REF, P0REF, T0REF, K0REF
        if setrefs==True:
            self.R0REF = rho0
            self.P0REF = P0
            self.T0REF = T0
            self.K0REF = K0
            print('R0REF = %g' % (self.R0REF))
            print('P0REF = %g' % (self.P0REF))
            print('T0REF = %g' % (self.T0REF))
            print('K0REF = %g' % (self.K0REF))
            ...
            if testout[i] == '  TWO-PHASE BOUNDARIES\n':
                ...
                while flag == 0:
                    if testout[j+i+4].find(' anphas') == 0:
                        ...
                        #RIC if includetension then allow exponents > 100
                        if includetension:
                            tmp=str.replace(tmp,'E','')
                            tmp=str.replace(tmp,' -',' M')
                            tmp=str.replace(tmp,'-','E-')
                            tmp=str.replace(tmp,'+','E+')
                            tmp=str.replace(tmp,' M',' -')
                        tmp3 = tmp[0:157]
                        tmp4 = list(tmp3.split())
                        #RIC if statement for includetension
                        # False is as before, else allows negative pressures
                        if includetension == False:
